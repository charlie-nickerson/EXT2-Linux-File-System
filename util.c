    ...
    for (i=0; i<NFD; i++){
        if (running->fd[i]){
            ...
            printf("[pfd]:   %d %6s  %4d     [%d, %d]\n", i, mode, cur->offset,
                    cur->minodePtr->dev, cur->minodePtr->ino);
        } else
...
int parentdir(char* pathn, char buf[512]){
    int i;
    strcpy(buf, pathn);
    for(i = 0; buf[i]; i++);
    while(i > -1)
...
int base_name(char* pathn, char* buf)
{
    if(pathn[0] == 0) { return -1; }
    int i = strlen(pathn);
    int j = 0;
    ...
    if(pathn[i] == '/'){
        i++;
        while(pathn[i]) { buf[j++] = pathn[i++]; }
        buf[j] = 0;
        return 1;
    }
    else
    {
        strncpy(buf, pathn, 512);
        return 1;
    }
}